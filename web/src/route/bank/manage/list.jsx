...
export default function BankList() {
  ...
  useEffect(() => { setTitle('渠道管理'); }, [setTitle]);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const resp = await post('/bank/manage/', new URLSearchParams({
...
function MenuIconButton(props) {
    ...
    try {
      ...
      await confirm({
        description: develop.disabled ?
          `确定要恢复该渠道吗？恢复后该渠道可正常使用。`
          :
          `确定要禁用该渠道吗？禁用后该渠道不可以继续使用，直到恢复为止，历史数据不受影响。`,
        confirmationText: develop.disabled ? '恢复' : '禁用',
        confirmationButtonProps: { color: 'warning' },
        contentProps: { p: 8 },
      });
      await post('/bank/manage/disable',
        new URLSearchParams({ uuid: develop.uuid })
      );
      enqueueSnackbar('渠道状态已更新成功', { variant: 'success' });
      requestRefresh();
    } catch (err) {
      if (err) {
        enqueueSnackbar(err.message);
  ...
  const onDeleteClick = async () => {
    try {
      ...
      await confirm({
        description: '确定要删除该渠道吗？删除后不能恢复，历史数据不受影响。',
        confirmationText: '删除',
        confirmationButtonProps: { color: 'error' },
      });
      const params = new URLSearchParams({ uuid: develop.uuid });
      await del('/bank/manage/delete?' + params.toString());
      enqueueSnackbar('渠道已被删除', { variant: 'success' });
      requestRefresh();
    } catch (err) {
      if (err) {
        enqueueSnackbar(err.message);
