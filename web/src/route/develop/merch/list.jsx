...
export default function DevelopList() {
  ...
  useEffect(() => { setTitle('商户拓展商'); }, [setTitle]);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const resp = await post('/develop/merch/', new URLSearchParams({
...
function MenuIconButton(props) {
    ...
    try {
      ...
      await confirm({
        description: develop.disabled ?
          `确定要恢复该拓展商吗？恢复后该拓展商可正常使用。`
          :
          `确定要禁用该拓展商吗？禁用后该拓展商不可以继续使用，直到恢复为止，历史数据不受影响。`,
        confirmationText: develop.disabled ? '恢复' : '禁用',
        confirmationButtonProps: { color: 'warning' },
        contentProps: { p: 8 },
      });
      await post('/develop/merch/disable',
        new URLSearchParams({ uuid: develop.uuid })
      );
      enqueueSnackbar('拓展商状态已更新成功', { variant: 'success' });
      requestRefresh();
    } catch (err) {
      if (err) {
        enqueueSnackbar(err.message);
  ...
  const onDeleteClick = async () => {
    try {
      ...
      await confirm({
        description: '确定要删除该拓展商吗？删除后不能恢复，历史数据不受影响。',
        confirmationText: '删除',
        confirmationButtonProps: { color: 'error' },
      });
      const params = new URLSearchParams({ uuid: develop.uuid });
      await del('/develop/merch/delete?' + params.toString());
      enqueueSnackbar('拓展商已被删除', { variant: 'success' });
      requestRefresh();
    } catch (err) {
      if (err) {
        enqueueSnackbar(err.message);
